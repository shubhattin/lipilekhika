<script lang="ts">
  ...
  onMount(() => {
    const prom = [
      preloadScriptData($typing_script_atom),
...
<div
  class="mt-12 flex min-h-[90vh] flex-col items-center justify-center overflow-hidden px-4 pt-4"
>
  <div class="mx-auto w-full max-w-6xl px-0 sm:px-6">
    ...
    >
      <!-- From / To Sections -->
      <div
        class="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-start"
      >
        <!-- From Section -->
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                >
                  <Icon
                    src={BiHelpCircle}
                    className="size-6 fill-sky-500 dark:fill-sky-400"
                  />
              ...
              <div class="flex items-center gap-2">
                <span
                  class="hidden text-xs text-muted-foreground sm:inline-block"
  ...
  {#if typing_modal_open}
    {#await import("./TypingHelper.svelte") then { default: TypingHelper }}
      <TypingHelper
        bind:open={typing_modal_open}
        script={$typing_script_atom}
      />
    {/await}
  {/if}
</div>

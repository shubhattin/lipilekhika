...
from test_stats import (  # ty:ignore[unresolved-import]
    get_assertion_count,
    get_file_assertion_counts,
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if HAS_RICH:
        ...
        table.add_row(
            " Assertions",
            f"[bold]{assertion_count:,}[/bold] [dim]passed ({assertion_count:,})[/dim]",
        )
        table.add_row(
            " Tests",
            f"[bold]{passed_count}[/bold] [dim]passed ({passed_count + failed_count + skipped_count})[/dim]",
        )
        ...
        if skipped_count > 0:
            table.add_row(
                ...
                f"[bold yellow]{skipped_count}[/bold yellow]",
            )
        ...
        print(
            f" Tests       {passed_count} passed ({passed_count + failed_count + skipped_count})"
        )

...
impl ScriptData {
  ...
  pub fn init_lookups(&mut self) {
    ...
    for (i, (text, _)) in attr.krama_text_arr.iter().enumerate() {
      if !krama_text_lookup.contains_key(text) {
        krama_text_lookup.insert(text.clone(), i);
      }
    }
    ...
    for (i, (text, _)) in attr.text_to_krama_map.iter().enumerate() {
      if !text_to_krama_lookup.contains_key(text) {
        text_to_krama_lookup.insert(text.clone(), i);
      }
    }
    ...
    for (i, (text, _)) in attr.typing_text_to_krama_map.iter().enumerate() {
      if !typing_text_to_krama_lookup.contains_key(text) {
        typing_text_to_krama_lookup.insert(text.clone(), i);
      }
    }
    ...
    for (i, (text, _, _)) in attr.custom_script_chars_arr.iter().enumerate() {
      if !custom_script_chars_lookup.contains_key(text) {
        custom_script_chars_lookup.insert(text.clone(), i);
      }
    }

...
import { sortArray, binarySearchLower } from './binary_search';
import { createSearchIndex, binarySearchLowerWithIndex } from './binary_search';
...
describe('binarySearchWithIndex', () => {
  describe('Number arrays', () => {
    it('should find existing element in number array', () => {
      ...
      arr.forEach((v, i) => {
        expect(binarySearchLowerWithIndex(arr, index, v)).toBe(i);
      });
    });
    ...
    it('should return -1 for non-existent element in number array', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 0)).toBe(-1);
      expect(binarySearchLowerWithIndex(arr, index, 4)).toBe(-1);
      expect(binarySearchLowerWithIndex(arr, index, 10)).toBe(-1);
    });
    ...
    it('should handle single element array', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 42)).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 41)).toBe(-1);
    });
    ...
    it('should handle empty array', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 5)).toBe(-1);
    });
  });
  ...
  describe('Hindi string arrays', () => {
    it('should find existing element in Hindi string array', () => {
      ...
      const foundIdx1 = binarySearchLowerWithIndex(arr, index, 'नमस्ते');
      expect(arr[foundIdx1]).toBe('नमस्ते');

      const foundIdx2 = binarySearchLowerWithIndex(arr, index, 'धन्यवाद');
    ...
    it('should return -1 for non-existent element in Hindi string array', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 'दा')).toBe(-1);
      expect(binarySearchLowerWithIndex(arr, index, 'शुभ')).toBe(-1);
    });
  });
  ...
  describe('Mixed Array with blank strings', () => {
    it('should handle array with one empty string', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, '')).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 'a')).toBe(-1);
      expect(binarySearchLowerWithIndex(arr, index, 'नमस्ते')).toBe(-1);
    });
    ...
    it('should handle array with two empty strings', () => {
      ...
      const foundIdx = binarySearchLowerWithIndex(arr, index, '');
    ...
    it('should handle empty strings mixed with Hindi strings', () => {
      ...
      const foundIdx1 = binarySearchLowerWithIndex(arr, index, '');
      expect(foundIdx1).toBeGreaterThanOrEqual(0);
      expect(arr[foundIdx1]).toBe('');

      // Verify Hindi strings can still be found
      expect(binarySearchLowerWithIndex(arr, index, 'नमस्ते')).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 'धन्यवाद')).toBe(2);
      expect(binarySearchLowerWithIndex(arr, index, 'स्वागत')).toBe(4);
    });
    ...
    it('should handle empty strings mixed with Telugu strings', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 'నమస్కారం')).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 'ధన్యవాదాలు')).toBe(2);
      expect(binarySearchLowerWithIndex(arr, index, 'స్వాగతం')).toBe(4);
    });
    ...
    it('should handle empty strings mixed with both Hindi and Telugu strings', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 'नमस्ते')).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 'నమస్కారం')).toBe(2);
      expect(binarySearchLowerWithIndex(arr, index, 'धन्यवाद')).toBe(4);
    });
    ...
    it('should handle array ending with empty string', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 'नमस्ते')).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 'धन्यवाद')).toBe(1);
    });
    ...
    it('should return -1 for non-empty string when searching in array with only empty strings', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 'a')).toBe(-1);
      expect(binarySearchLowerWithIndex(arr, index, 'नमस्ते')).toBe(-1);
      expect(binarySearchLowerWithIndex(arr, index, ' ')).toBe(-1);
    });
  });
  ...
  describe('Lower bound behavior with duplicates', () => {
    it('should return lower bound index for duplicates in unsorted array', () => {
      ...
      const foundIdx1 = binarySearchLowerWithIndex(arr, index, 1);
      ...
      const foundIdx3 = binarySearchLowerWithIndex(arr, index, 3);
    ...
    it('should return lower bound for array with all same values', () => {
      ...
      const foundIdx = binarySearchLowerWithIndex(arr, index, 5);
    ...
    it('should return lower bound for consecutive duplicates', () => {
      ...
      const foundIdx = binarySearchLowerWithIndex(arr, index, 2);
    ...
    it('should return lower bound with custom accessor for duplicates', () => {
      ...
      const foundIdx = binarySearchLowerWithIndex(arr, index, 'a', { accessor });
  ...
  describe('Edge cases', () => {
    it('should handle array with duplicate values', () => {
      ...
      const foundIdx = binarySearchLowerWithIndex(arr, index, 3);
      expect(foundIdx).toBeGreaterThanOrEqual(0);
      expect(arr[foundIdx]).toBe(3);

      const foundIdx2 = binarySearchLowerWithIndex(arr, index, 1);
    ...
    it('should handle already sorted array', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 3)).toBe(2);
      expect(binarySearchLowerWithIndex(arr, index, 1)).toBe(0);
      expect(binarySearchLowerWithIndex(arr, index, 5)).toBe(4);
    });
    ...
    it('should handle reverse sorted array', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 3)).toBe(2);
      expect(binarySearchLowerWithIndex(arr, index, 1)).toBe(4);
      expect(binarySearchLowerWithIndex(arr, index, 5)).toBe(0);
    });
  });
  ...
  describe('Custom Array Type', () => {
    it('should handle custom array type', () => {
      ...
      expect(binarySearchLowerWithIndex(arr, index, 'a', { accessor: (arr, i) => arr[i][0] })).toBe(
        2
      );
      expect(binarySearchLowerWithIndex(arr, index, 'c', { accessor: (arr, i) => arr[i][0] })).toBe(
        1
      );
      expect(binarySearchLowerWithIndex(arr, index, 'j', { accessor: (arr, i) => arr[i][0] })).toBe(
        -1
      );
      expect(binarySearchLowerWithIndex(arr, index, 'z', { accessor: (arr, i) => arr[i][0] })).toBe(
        0
      );
    });
  });
...
describe('binarySearch', () => {
  it('should find target in sorted number array', () => {
    const arr = [1, 2, 3, 4, 5];
    expect(binarySearchLower(arr, 3)).toBe(2);
    expect(binarySearchLower(arr, 1)).toBe(0);
    expect(binarySearchLower(arr, 5)).toBe(4);
  });

  it('should return -1 for missing element in sorted number array', () => {
    const arr = [1, 2, 3, 4, 5];
    expect(binarySearchLower(arr, 0)).toBe(-1);
    expect(binarySearchLower(arr, 6)).toBe(-1);
  });

  it('should work with custom accessor', () => {
    const arr = [{ v: 1 }, { v: 3 }, { v: 7 }];
    expect(binarySearchLower(arr, 3, { accessor: (arr, i) => arr[i].v })).toBe(1);
    expect(binarySearchLower(arr, 4, { accessor: (arr, i) => arr[i].v })).toBe(-1);
  });

  it('should work with custom compareFn (descending search)', () => {
    const arr = [5, 4, 3, 2, 1];
    expect(binarySearchLower(arr, 2, { compareFn: (a, b) => b - a })).toBe(3);
    expect(binarySearchLower(arr, 6, { compareFn: (a, b) => b - a })).toBe(-1);
  });

  it('should handle sorted string arrays', () => {
    const arr = ['apple', 'banana', 'carrot'];
    expect(binarySearchLower(arr, 'banana')).toBe(1);
    expect(binarySearchLower(arr, 'zzz')).toBe(-1);
  });

  it('should handle single element array', () => {
    expect(binarySearchLower([42], 42)).toBe(0);
    expect(binarySearchLower([42], 24)).toBe(-1);
  });

  it('should handle empty array', () => {
    expect(binarySearchLower([], 42)).toBe(-1);
  });

  it('should return the first (lower bound) index for duplicate elements', () => {
    // Test duplicates in the middle
    const arr1 = [1, 2, 2, 2, 3];
    expect(binarySearchLower(arr1, 2)).toBe(1); // First occurrence of 2

    // Test duplicates at the start
    const arr2 = [1, 1, 1, 2, 3];
    expect(binarySearchLower(arr2, 1)).toBe(0); // First occurrence of 1

    // Test duplicates at the end
    const arr3 = [1, 2, 3, 3, 3];
    expect(binarySearchLower(arr3, 3)).toBe(2); // First occurrence of 3

    // Test all same elements
    const arr4 = [5, 5, 5, 5, 5];
    expect(binarySearchLower(arr4, 5)).toBe(0); // First occurrence

    // Test two duplicates
    const arr5 = [1, 2, 2, 3];
    expect(binarySearchLower(arr5, 2)).toBe(1); // First occurrence of 2
  });

  it('should return lower bound with custom accessor for duplicates', () => {
    const arr = [{ v: 1 }, { v: 2 }, { v: 2 }, { v: 2 }, { v: 3 }];
    const idx = binarySearchLower(arr, 2, { accessor: (arr, i) => arr[i].v });

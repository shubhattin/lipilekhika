...
import { build_script_data_cache_ } from '../transliteration/helpers';
...
export const getScriptData = async (script_name: script_list_type): Promise<OutputScriptData> => {
  if (IS_UMD_BUILD_MODE) {
    ...
    data.then((data) => build_script_data_cache_(data));
    return data;
  }
  ...
  ESM_SCRIPT_DATA_PROMISE_CACHE[script_name]!.then((data) => build_script_data_cache_(data));

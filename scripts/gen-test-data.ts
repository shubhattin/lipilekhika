...
const devanagari_non_brahmic_scripts = async () => {
  ...
  const FROM_SCRIPT = 'Devanagari';
  const NON_BRAHMI_SCRIPTS = ['Normal', 'Romanized'] satisfies script_list_type[];
  const out_test_data: TestData[] = [];
  let index = 0;
  for (const input of DEVANAGARI_INPUTS) {
    for (const non_brahmic_script of NON_BRAHMI_SCRIPTS) {
      const output = await old_lipi_parivartak(input, FROM_SCRIPT, non_brahmic_script);
      out_test_data.push({
        index: index++,
        from: FROM_SCRIPT,
...
async function main() {
  ...
  await Promise.all([devangari_other_brahmic_scripts(), devanagari_non_brahmic_scripts()]);
}

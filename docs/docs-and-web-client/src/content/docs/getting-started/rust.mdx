...
- **`ctx.update_use_native_numerals(boolean)`** - Enable/disable native numerals (e.g., १० instead of 10)
- **`ctx.update_include_inherent_vowel(boolean)`** - Enable/disable inherent vowel inclusion (schwa deletion)
